...
class ArmControlRouter(Node):
    def __init__(self):
        ...
        self.ready = False
        self.create_timer(3.0, self.mark_ready, callback_group=self.callback_group)

        self.get_logger().info('🛠️ ArmControlRouter initialized.')

    def mark_ready(self):
        self.ready = True
        self.get_logger().info('✅ ArmControlRouter is READY to receive tasks.')

    def text_callback(self, msg):
        if self.text_receiving:
            self.get_logger().info(
                f"📨 Text coordinate received: ({msg.position.x:.3f}, {msg.position.y:.3f}, {msg.position.z:.3f})"
            )
            self.latest_text_coordinate = msg
        else:
            self.get_logger().warn("Text coordinate ignored (flag off).")
    ...
    def run_task_service(self, request, response):
        if not self.ready:
            ...
            response.message = "Router not ready yet. Please wait a few seconds."
            return response
    ...
    def wait_for_service_ready(self, client, name, timeout=10.0):
        start = time.time()
    ...
    def send_named_pose(self, name, response):
        if not self.wait_for_service_ready(self.named_pose_client, "named_pose_service"):
            response.success = False
    ...
    def send_auto_pose(self, response):
        if not self.wait_for_service_ready(self.auto_pose_client, "auto_pose_service"):
            response.success = False
    ...
    def send_gripper(self, close, response):
        if not self.wait_for_service_ready(self.gripper_client, "gripper_control_service"):
            response.success = False
